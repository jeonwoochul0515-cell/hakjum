...
function fallbackExploreRecommend(interest: string): AIExploreResult {
  ...
  const scored = popularMajors.map((major) => {
    ...
    const keywordMap: Record<string, string[]> = {
      '컴퓨터공학과': ['코딩', '프로그래밍', '개발', 'ai', '인공지능', '소프트웨어', '앱', '게임', '해킹', '보안', '데이터', '컴퓨터'],
      '간호학과': ['간호', '병원', '환자', '돌봄', '의료', '건강'],
      '의예과(의학과)': ['의사', '의대', '의학', '수술', '치료'],
      '경영학과': ['경영', '사업', '창업', '마케팅', '회사', '기업', 'ceo', '비즈니스'],
      '법학과': ['법', '변호사', '판사', '검사', '법률', '정의'],
      '교육학과': ['교사', '선생님', '교육', '가르치', '아이', '학생'],
      '전기·전자공학과': ['전자', '반도체', '전기', '회로', '로봇', '자동차'],
      '약학과': ['약사', '약학', '제약', '약', '신약'],
      '국어국문학과': ['글쓰기', '문학', '작가', '소설', '시', '국어', '한국어', '기자'],
      '디자인학과(시각/산업)': ['디자인', '미술', '그림', '예술', 'ux', 'ui', '브랜드', '영상'],
    };
  ...
  return {
    recommendations,
    summary: `"${interest}" 관심사를 바탕으로 관련 학과를 찾아봤어요. 더 정확한 추천을 위해 AI 분석을 이용해보세요.`,
    source: 'fallback',
  };
}
