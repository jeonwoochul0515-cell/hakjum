/**
 * 경기(안성) 고교학점제 PDF → Claude AI 구조화 추출 스크립트
 *
 * 입력: data/gyeonggi/pdfs/*.pdf
 * 출력: data/gyeonggi/extracted/<basename>.json
 *
 * busan-pdf-extract.ts 와 동일한 구조 — 출처/카테고리만 경기로 변경.
 *
 * 실행:
 *   ANTHROPIC_API_KEY=... npx tsx scripts/gyeonggi-pdf-extract.ts
 *   ANTHROPIC_API_KEY=... npx tsx scripts/gyeonggi-pdf-extract.ts --file <파일명>
 *   ANTHROPIC_API_KEY=... npx tsx scripts/gyeonggi-pdf-extract.ts --max-files 1
 *   npx tsx scripts/gyeonggi-pdf-extract.ts --dry-run
 */
...
function buildUserPrompt(title: string, pdfText: string): string {
  const trimmed = pdfText.length > MAX_TEXT_LEN
    ...
    : pdfText;
  return `당신은 경기도교육청(안성교육지원청) 고교학점제지원센터 안내자료 PDF에서 학생 진로/과목 선택 정보를 추출하는 전문가입니다.

자료 제목: ${title}
출처: 경기도안성교육지원청 고교학점제지원센터 (hscredit.goean.kr)
...
3. subjects:
   - "area" 가능 값: "공통과목", "일반선택", "진로선택", "융합선택", "경기형선택과목", "전문교과", "기타"
   - 경기/안성 자체 운영 과목(공동교육과정, 거점학교 개설과목 등)이 명시되면 정확히 표기.
   - 학년/학기 정보가 있으면 채우고, 없으면 null.
4. schools: 거점학교, 공동교육과정 운영교, 안성 지역 협력학교 등. PDF에 학교명이 명시된 경우만.
...
async function main() {
  ...
  const outDir = path.join(projectRoot, 'data', 'gyeonggi', 'extracted');
  const manifestPath = path.join(projectRoot, 'data', 'gyeonggi', '_meta', 'manifest.json');
  fs.mkdirSync(outDir, { recursive: true });
  if (!fs.existsSync(pdfDir)) {
    console.error(`PDF 디렉터리가 없습니다: ${pdfDir}\n먼저 npx tsx scripts/gyeonggi-pdf-collect.ts 실행 또는 수동 다운로드 후 재실행.`);
    process.exit(1);
  }
